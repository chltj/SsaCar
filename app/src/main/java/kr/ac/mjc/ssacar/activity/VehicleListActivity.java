...
public class VehicleListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 출발지 정보 받기 (CallHere에서)
        startPlaceName = receivedIntent.getStringExtra("start_place_name");
        startAddress = receivedIntent.getStringExtra("start_address");
        startLatitude = receivedIntent.getDoubleExtra("start_latitude", 0.0);
        startLongitude = receivedIntent.getDoubleExtra("start_longitude", 0.0);

        // 도착지 정보 받기 (CallReturn에서)
        endPlaceName = receivedIntent.getStringExtra("end_place_name");
        endAddress = receivedIntent.getStringExtra("end_address");
        endLatitude = receivedIntent.getDoubleExtra("end_latitude", 0.0);
        endLongitude = receivedIntent.getDoubleExtra("end_longitude", 0.0);

        // 시간 정보
        departureTime = receivedIntent.getStringExtra("departure_time");
        arrivalTime = receivedIntent.getStringExtra("arrival_time");
        selectedMonth=receivedIntent.getStringExtra("selected_month");

        // 이전 버전과의 호환성을 위한 처리
        if (startPlaceName == null) {
            startPlaceName = receivedIntent.getStringExtra("place_name");
            startAddress = receivedIntent.getStringExtra("address");
            startLatitude = receivedIntent.getDoubleExtra("latitude", 0.0);
            startLongitude = receivedIntent.getDoubleExtra("longitude", 0.0);
        }
    ...
    private void initViews() {
        ...
        // 디버그 로그 추가
        android.util.Log.d("VehicleListActivity", "출발지: " + startPlaceName);
        android.util.Log.d("VehicleListActivity", "도착지: " + endPlaceName);
        android.util.Log.d("VehicleListActivity", "출발시간: " + departureTime);
        android.util.Log.d("VehicleListActivity", "도착시간: " + arrivalTime);

        if (startAddress != null && endAddress != null) {
            // 출발지와 도착지가 모두 있는 경우
            locationInfo.append("대여장소\n");

            // 출발지와 출발시간을 한 줄에
            locationInfo.append("출발: ").append(startAddress+"\n");
            if (departureTime != null) {
                locationInfo.append(" (").append(departureTime).append(")");
            }
            locationInfo.append("\n");
            ...
            if (arrivalTime != null) {
                locationInfo.append(" (").append(arrivalTime).append(")");
            }
            locationInfo.append("\n");
            ...
            // 아무 정보도 없는 경우
            locationInfo.append("렌트기간: ").append(selectedMonth).append("");
        }

        locationTimeInfoTextView.setText(locationInfo.toString());

        // 추가 디버그: 실제 표시되는 텍스트 확인
        android.util.Log.d("VehicleListActivity", "표시되는 텍스트: " + locationInfo.toString());

        selectCompleteButton.setOnClickListener(v -> {
            if (selectedCar != null) {
                Intent intent = new Intent(VehicleListActivity.this, PaymentActivity.class);
                intent.putExtra("selected_vehicle", selectedCar);

                // 출발지 정보 전달
                intent.putExtra("start_place_name", startPlaceName);
                ...
                // 도착지 정보 전달
                intent.putExtra("end_place_name", endPlaceName);
                ...
                // 이전 버전과의 호환성
                intent.putExtra("place_name", startPlaceName);
                                    ...
                                    try {
                                        if (currentIndex < carList.size()) {
                                            ...
                                            if (targetCar != null) {
                                                ...
                                                vehicleAdapter.notifyItemChanged(currentIndex);
